#!/usr/bin/env bash
# Engine search step: MS-GF+ (Java; jar on PATH in the image). MS-GF+ uses
# instrument presets for fragment tolerance (high_res for the Orbitrap
# QExactiveHF), so no fragment-tol flag.
run_search() {
